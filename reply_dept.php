...
$query= mysql_query("SELECT ID, commentcontent, datecommented, timecommented, comment_dept.userLRN, Fname, Lname FROM comment_dept INNER JOIN profile ON comment_dept.userLRN = profile.userLRN WHERE ID=".$commentID."");
while($rows = mysql_fetch_array($query)){	
...
<b><font color="#00679b">
	<?php 
	echo "<a href=\"profile_explore.php?userLRN=$exploreLRN\" style='text-decoration: none;'>" . $rows[5] . " " . $rows[6] . " </a></font></b><font color='a599ae' size='2'>@ " . $rows[2] . " " . $rows[3] . "</font><br>";
	echo $rows[1] . "<br>";
	echo "<hr>";
	?>
	<input type="hidden" name="postno" value="<?php echo $commentID; $_SESSION["commentID"] = $commentID;?>">
	<?php
	$query1 = mysql_query("SELECT replycontent, datereplied, timereplied, Fname, Lname, reply_dept.ID, profile.userLRN  FROM reply_dept INNER JOIN profile on profile.userLRN=reply_dept.userLRN WHERE commentID='$commentID'") or die("ERROR QUERY");
...
<?php
		...
		echo "<a href=\"profile_explore.php?userLRN=$exploreLRN\" style='text-decoration: none;'>" . $rows[3] . " " . $rows[4] . " </a></b></font><font color='a599ae' size='2'>@ " . $rows[1] . " " . $rows[2] . "</font><br>";
		echo $rows[0];
		echo "<br>";
		if($rows[6] == $lrn){
			echo "<a href='deletereply_dept.php?ID=$replyID'>Delete Reply</a></div>";
		}
	}
?>
...
if(isset($_POST["reply"])){
	...
	$replyquery = mysql_query("INSERT INTO reply_dept (replycontent, datereplied, timereplied, commentID, userLRN) VALUES('$reply','$datereplied', '$timereplied','$commentID', '$lrn')") or die("ERROR QUERY");
	echo("<script>location.href='reply_dept.php?ID=$commentID';</script>");
}
?>
</html>
</body>
