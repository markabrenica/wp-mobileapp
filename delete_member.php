...
<li role="presentation" class="w3-small"><a href="message.php" style="color:#66757f;"><span class="fa fa-envelope" aria-hidden="true"></span></a></li>
	...
	<p id="ID" hidden> <?php echo $groupID; ?> </p>
<?php
if(isset($_POST["btnYes"])){
$queryDeletemember = mysql_query("DELETE FROM groupmember WHERE userID='$userID' AND groupID='$groupID'");
?>
<script>
	swal({
    title: "Confirmation",
  ...
  function(isConfirm){
    if (isConfirm){
    	location.href = 'group_explore.php?ID=' + document.getElementById("ID").innerHTML;
    }
  });
...
echo("<script>location.href='group_explore.php?ID=$groupID';</script>");
}
?>
<form action="" method="POST">

<?php
$queryShow = mysql_query("SELECT userID, groupID, Fname, Lname FROM groupmember INNER JOIN profile ON groupmember.userID = profile.userLRN WHERE groupID='$groupID' AND userID='$userID'") or die('ERROR QUERY');
while($rows = mysql_fetch_array($queryShow)){
?>
<h4> Are you sure you want to remove <?php echo $rows[2] . " " . $rows[3]; ?> to your group? </h4>
