...
$query= mysql_query("SELECT ID, commentcontent, datecommented, timecommented, comments.userLRN, Fname, Lname FROM comments INNER JOIN profile ON comments.userLRN = profile.userLRN WHERE ID=".$commentID."");
...
<b><font color="#00679b">
	...
	<?php
	$query1 = mysql_query("SELECT replycontent, datereplied, timereplied, Fname, Lname, reply.ID, profile.userLRN  FROM reply INNER JOIN profile on profile.userLRN=reply.userLRN WHERE commentID='$commentID'") or die("ERROR QUERY");
...
<?php
		...
		echo "<br>";
		if($rows[6] == $lrn){
			echo "<a href='deletereply.php?ID=$replyID'>Delete Reply</a></div>";
		}
	}
?>
...
if(isset($_POST["reply"])){
	...
	$replyquery = mysql_query("INSERT INTO reply (replycontent, datereplied, timereplied, commentID, userLRN) VALUES('$reply','$datereplied', '$timereplied','$commentID', '$lrn')") or die("ERROR QUERY");
	echo("<script>location.href='reply.php?ID=$commentID';</script>");
}
?>
</html>
</body>
