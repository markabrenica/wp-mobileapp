...
$query= mysql_query("SELECT ID, commentcontent, datecommented, timecommented, comment_announcement.userLRN, Fname, Lname FROM comment_announcement INNER JOIN profile ON comment_announcement.userLRN = profile.userLRN WHERE ID=".$commentID."");
...
<b><font color="#00679b">
	...
	<?php
	$query1 = mysql_query("SELECT replycontent, datereplied, timereplied, Fname, Lname, reply_announcement.ID, profile.userLRN  FROM reply_announcement INNER JOIN profile on profile.userLRN=reply_announcement.userLRN WHERE commentID='$commentID'") or die("ERROR QUERY");
...
if(isset($_POST["reply"])){
	...
	$replyquery = mysql_query("INSERT INTO reply_announcement (replycontent, datereplied, timereplied, commentID, userLRN) VALUES('$reply','$datereplied', '$timereplied','$commentID', '$lrn')") or die("ERROR QUERY");
	echo("<script>location.href='reply_ann.php?ID=$commentID';</script>");
}
?>
</html>
</body>
