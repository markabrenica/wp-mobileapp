...
<body>
	<?php
		if(isset($_POST["btnSAVE"])){
			...
			$link = mysql_real_escape_string($_POST["videolink"]);
			$newlink = substr($link, 31);
			date_default_timezone_set("Asia/Manila");
			$dateshared = date("F j, Y");
			$timeshared = date("h:i:s a");
			if(!empty($link)){
				$insert = mysql_query("INSERT INTO videos VALUES('','$caption','$newlink','$dateshared','$timeshared', '$lrn')") or die("ERROR QUERY");
			}
			else{
				echo "<script> swal('Warning!', 'Link shared is not valid!', 'warning');</script>";
			}
	...
	?>
	<div class="container">
	<form method="POST" action="add-video.php">
...
<input type="text" name="videolink" placeholder="Youtube link"><br>
<input type="submit" name="btnSAVE" value="Share"><br>

<?php
$query = mysql_query("SELECT * FROM videos") or die("ERROR QUERY");
while($rows = mysql_fetch_array($query)){
	echo $rows[1] . " " . $rows[3] . " @ $rows[4] <br>";
?>

<iframe width="360" height="250" src="http://www.youtube.com/watch?v=<?php echo $rows[2]; ?>" frameborder="0" allowfullscreen></iframe><br>
<?php
}
?>
