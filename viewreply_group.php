...
$query= mysql_query("SELECT ID, content, dateposted, timeposted, Fname, Lname, profile.userLRN FROM grouppost INNER JOIN profile ON grouppost.userID = profile.userLRN WHERE grouppost.ID=".$id."") or die("ERROR QUERY");
...
<div style="width: 100%;">
	...
	echo $rows1[4] . " " . $rows1[5] . "</a> </font></b><font color='a599ae' size='2'>@ " . $rows1[2] . " " . $rows1[3] . "</font><br>";
	echo $rows1[1] . "<br>";
	echo "<hr>";
	?>
	<input type="hidden" name="postno" value="<?php echo $id; $_SESSION["postID"] = $id;?>">
	<?php
	$query1 = mysql_query("SELECT commentcontent, datecommented, timecommented, Fname, Lname, commentgroup.ID, profile.userLRN  FROM commentgroup INNER JOIN profile on profile.userLRN=commentgroup.userID WHERE postID='$id'") or die("ERROR QUERY");
	while($rows = mysql_fetch_array($query1)){
		...
		echo $rows[0] . " <br> ";
		$count = 0;
		$querycountreply = mysql_query("SELECT replycontent, datereplied, timereplied, Fname, Lname, reply_group.ID  FROM reply_group INNER JOIN profile on profile.userLRN=reply_group.userID WHERE commentID='$id1'") or die("ERROR QUERY");
		while($rows = mysql_fetch_array($querycountreply)){
			$count = $count + 1;
		}
		echo "<i>" . $count . " Replies </i><br>";
		echo "<a href='reply_group.php?ID=$replyID'>Reply</a> ";

		if($lrn == $rows1["userLRN"]){
echo "| <a href='deletecomment_group.php?ID=$replyID'>Delete Comment</a>";
...
if(isset($_POST["reply"])){
	...
	$replyquery = mysql_query("INSERT INTO commentgroup (commentcontent, datecommented, timecommented, postID, userID) VALUES('$reply','$datereplied', '$timereplied','$postID', '$lrn')") or die("ERROR QUERY");
	echo("<script>location.href='viewreply_group.php?ID=$postID';</script>");
}
?>
</form>
</body>
