...
$query= mysql_query("SELECT ID, content, dateposted, timeposted, announcement.userLRN, Fname, Lname FROM announcement INNER JOIN profile ON announcement.userLRN = profile.userLRN WHERE ID=".$annID."") or die("ERROR QUERY");
...
<?php
	...
	echo "<hr>";
	?>
	<input type="hidden" name="postno" value="<?php echo $annID; $_SESSION["annID"] = $annID;?>">
<?php
	$query1 = mysql_query("SELECT commentcontent, datecommented, timecommented, Fname, Lname, comment_announcement.ID, profile.userLRN  FROM comment_announcement INNER JOIN profile on profile.userLRN=comment_announcement.userLRN WHERE annID='$annID'") or die("ERROR QUERY");
	while($rows = mysql_fetch_array($query1)){
		...
		echo $rows[0] . " <br>";
		$count = 0;
		$querycountreply = mysql_query("SELECT replycontent, datereplied, timereplied, Fname, Lname, reply_announcement.ID FROM reply_announcement INNER JOIN profile on profile.userLRN=reply_announcement.userLRN WHERE commentID='$replyID' ORDER BY commentID") or die("ERROR QUERY");
		while($rows = mysql_fetch_array($querycountreply)){
			$count = $count + 1;
		}
		echo "<i>" . $count . " Replies </i><br>";
		echo "<a href='reply_ann.php?ID=$replyID'>Reply</a> ";

		if($lrn == $rows1["userLRN"]){
echo "| <a href='deletecomment_ann.php?ID=$replyID'>Delete Comment</a>";
...
if(isset($_POST["reply"])){
	...
	$replyquery = mysql_query("INSERT INTO comment_announcement (commentcontent, datecommented, timecommented, annID, userLRN) VALUES('$reply','$datereplied', '$timereplied','$annID', '$lrn')") or die("ERROR QUERY");
	echo("<script>location.href='viewcomment_ann.php?ID=$annID';</script>");
}
?>
</form>
</body>
