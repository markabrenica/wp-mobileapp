...
$query= mysql_query("SELECT ID, content, dateposted, timeposted, department.userLRN, Fname, Lname FROM department INNER JOIN profile ON department.userLRN = profile.userLRN WHERE ID=".$postID."");
...
<?php
	$query1 = mysql_query("SELECT commentcontent, datecommented, timecommented, Fname, Lname, comment_dept.ID, profile.userLRN FROM comment_dept INNER JOIN profile on profile.userLRN=comment_dept.userLRN WHERE postID='$postID'") or die("ERROR QUERY");
	while($rows = mysql_fetch_array($query1)){
		...
		echo "<font color='#00679b'><b><a href=\"profile_explore.php?userLRN=$exploreLRN\" style='text-decoration: none;'>";
		echo $rows[3]." ".$rows[4]."</a></b></font><font color='a599ae' size='2'> @ ".$rows[1]." ".$rows[2]."</font><br>";
		echo $rows[0] . " <br> ";
		$count = 0;
		$querycountreply = mysql_query("SELECT replycontent, datereplied, timereplied, Fname, Lname, reply_dept.ID  FROM reply_dept INNER JOIN profile on profile.userLRN=reply_dept.userLRN WHERE commentID='$id' ORDER BY commentID") or die("ERROR QUERY");
		while($rows = mysql_fetch_array($querycountreply)){
			$count = $count + 1;
		}
		echo "<i>" . $count . " Replies </i><br>";
		echo "<a href='reply_dept.php?ID=$replyID'>Reply</a> ";

		if($lrn == $rows1["userLRN"]){
echo "| <a href='deletecomment_dept.php?ID=$replyID'>Delete Comment</a>";
...
if(isset($_POST["reply"])){
	...
	$replyquery = mysql_query("INSERT INTO comment_dept (commentcontent, datecommented, timecommented, postID, userLRN) VALUES('$reply','$datereplied', '$timereplied','$postID', '$lrn')") or die("ERROR QUERY");
	echo("<script>location.href='viewreply_dept.php?ID=$postID';</script>");
