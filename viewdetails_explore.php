...
<body>
<?php
$query = mysql_query("SELECT `profile`.`userLRN`, `profile`.`Fname`, `profile`.`Lname`, `profile`.`Mname`, `degree`.`userDegree`, `profile`.`Sex`, `profile`.`Address`, `profile`.`Birthdate`, `profile`.`Contactno`,`profile`.`CivilStatus`, `degree`.`Yearno`, `users`.`userLevel` FROM `profile` INNER JOIN `degree` ON `profile`.`userLRN` = `degree`.`userLRN` INNER JOIN `users` ON `users`.`userLRN`=`degree`.`userLRN` WHERE `profile`.`userLRN`='$explorelrn'") or die("ERROR QUERY");
...
<li role="presentation" class="w3-small"><a href="message.php" style="color:#66757f;"><span class="fa fa-envelope" aria-hidden="true"></span></a></li>
	   <li role="presentation" class="w3-small"><a href="pictures.php" style="color:#66757f;"><span class="fa fa-photo" aria-hidden="true"></span></a></li>
		...
		<center>
		<?php
$queryPic = mysql_query("SELECT * FROM profilepic WHERE userLRN='$explorelrn' ORDER BY dateuploaded DESC, timeuploaded DESC");
                     ...
                     <div class="row-fluid">
                        <div class="block">
                            ...
                            <div class="block-content collapse in">
                                <div class="span12">
								<div>
											<a href="profile_explore.php?userLRN=<?php echo $explorelrn; ?>" class="btn"><i class="icon-arrow-left"></i> Back</a>
											</div>
										<div class="span12">
										...
										<tr>
										<td>Year:</td>
										<td><span style="color:skyblue;"><?php echo $rowinfo["Yearno"]; }?></span></td>
										</tr>
										<?php
										}
										if($rowinfo[11] == "faculty"){
										?>
										<tr>
										<td>Department: </td>
										<td><span style="color:skyblue;"><?php echo $rowinfo["userDegree"];?></span></td>
