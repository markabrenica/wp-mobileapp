...
$query = mysql_query("SELECT path, dateuploaded, timeuploaded, userLRN FROM profilepic WHERE picID='$id' ORDER BY dateuploaded DESC, timeuploaded DESC") or DIE("ERROR IN QUERY");
					while($rowsinfo = mysql_fetch_array($query)){
						$filename = basename($rowsinfo[0]);
...
<div id="myAlert" class="modal show" style="margin-top: 70px;">
					...
					<div class="modal-body">
						...
						<br><br>
						Uploaded: <?php echo $rowsinfo["dateuploaded"] . " @ " . $rowsinfo["timeuploaded"];?>
						</center>
					</div>
					<div class="modal-footer">
						<a class="btn btn-primary" href="<?php echo $source; ?>">View Full size</a>
						<a class="btn" href="profilepic.php">Back to album</a>
					</div>
				</div>
				<?php
			}
