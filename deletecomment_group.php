...
$query = mysql_query("DELETE FROM commentgroup WHERE ID='$replyID'") or die("ERROR QUERY");
$query = mysql_query("DELETE FROM reply_group WHERE commentID='$replyID'") or die("ERROR QUERY");
?>
<script>
	var num = "";
  ...
  function(isConfirm){
    if (isConfirm){
    	location.href = 'viewreply_group.php?ID=' + document.getElementById("ID").innerHTML;
    }
  });
...
echo("<script>location.href='viewreply_group.php?ID=$postIDnum';</script>");
}
if(!isset($_POST["btnYes"])){
?>
