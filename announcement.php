...
<li role="presentation" class="w3-small"><a href="message.php" style="color:#66757f;"><span class="fa fa-envelope" aria-hidden="true"></span></a></li>
	...
	<li role="presentation" class="w3-medium"><a href="announcement.php" style="color:#66757f;"><span class="fa fa-list-alt" aria-hidden="true"></span></a></li>
	...
	<div class="se-pre-con"></div>
		...
		<div class="well">
		<?php
		$query = mysql_query("SELECT announcement.content, announcement.dateposted, announcement.timeposted, profile.Fname, profile.Lname, announcement.ID, profile.userLRN FROM announcement INNER JOIN profile ON announcement.userLRN=profile.userLRN ORDER BY ID DESC") or die("ERROR QUERY");
		while($rows = mysql_fetch_array($query)){
			...
			?>
		<b><font color="#00679b"><a style="text-decoration: none;" href="profile_explore.php?userLRN=<?php echo $exploreLRN; ?>"><?php echo $rows["Fname"] . " " . $rows["Lname"] . " </a></font></b> <font color='a599ae' size='2'>@ " . $rows["dateposted"] . " " . $rows["timeposted"]; ?></p></font>
		</font><font size="2"><p><?php echo $rows["content"]; ?> </p>
		<hr>
		<div class="btn-group" role="group" aria-label="buttongroup">
		<a href="viewcomment_ann.php?ID=<?php echo $id; ?>" data-toggle="modal" class="btn btn-default"><span class="icon-comment" aria-hidden="true"></span> 
			<?php
			$countquery = mysql_query("SELECT * FROM comment_announcement WHERE annID='$id'");
			$count = 0;
			while($rows1 = mysql_fetch_array($countquery)){
				$count = $count + 1;
			}
			echo $count;
			?>
	...
	<?php
		if($rows["userLRN"] == $lrn){
			?>
		<a href="editann.php?ID=<?php echo $id; ?>" data-toggle="modal" class="btn btn-default"><span class="icon-edit" aria-hidden="true"></span></a>
		<a href="deleteann.php?postID=<?php echo $id; ?>" data-toggle="modal" class="btn btn-default"><span class="icon-trash" aria-hidden="true"></span></a>
	</div>
		<?php
		}
		echo "<hr><br>";
