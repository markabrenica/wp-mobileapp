...
$query = mysql_query("DELETE FROM reply WHERE ID='$replyID'") or die("ERROR QUERY");
?>
<script>
	var num = "";
  ...
  function(isConfirm){
    if (isConfirm){
    	location.href = 'reply.php?ID=' + document.getElementById("ID").innerHTML;
    }
  });
...
echo("<script>location.href='reply.php?ID=$postIDnum';</script>");
}
if(!isset($_POST["btnYes"])){
?>
