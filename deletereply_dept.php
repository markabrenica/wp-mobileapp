...
//$query = mysql_query("DELETE FROM comments WHERE ID='$replyID'") or die("ERROR QUERY");
$query = mysql_query("DELETE FROM reply_dept WHERE ID='$replyID'") or die("ERROR QUERY");
?>
<script>
	var num = "";
	...
	swal({
    title: "Confirmation",
    text: "Reply has been successfully deleted!",
    type: "success",
  ...
  function(isConfirm){
    if (isConfirm){
    	location.href = 'reply_dept.php?ID=' + document.getElementById("ID").innerHTML;
    }
  });
...
echo("<script>location.href='reply_dept.php?ID=$postIDnum';</script>");
}
if(!isset($_POST["btnYes"])){
?>
