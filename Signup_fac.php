...
require "conn.php";
	...
	$deptErr =  $yearErr = $blockErr = $usernameErr = $passwordErr = "";
	$lrn = $fname = $lname = $mname = $contact = $gender = $bdate = $addr = $civil = "";
	$dept =  $year = $block = $username = $password = "";
 ...
 if(isset($_POST["btnADD"])){
	...
	if(!empty($_POST["lrn"]) && !empty($_POST["fname"]) && !empty($_POST["mname"]) && !empty($_POST["lname"])){
				if(!empty($_POST["gender"]) && !empty($_POST["contact"]) && !empty($_POST["currentAdd"]) && !empty($_POST["bdate"])){
					if(!empty($_POST["civil"]) && !empty($_POST["dept"]) && !empty($_POST["username"]) && !empty($_POST["password"])){
	//INSERT TO DATABASE
	//USER INFORMATION AND USER ACCOUNT
		$checkLRN = mysql_query("SELECT * FROM facultylrn WHERE LRN='".$lrn."'") or die("ERROR QUERY");
...
$insertAccount = mysql_query("INSERT INTO users(username, password, userlevel, userLRN) VALUES('$username', '$password', 'faculty', '$lrn')") or die("ERROR QUERY");
...
$insertDegree = mysql_query("INSERT INTO degree(userDegree, Yearno, userLRN) VALUES('$dept','$year','$lrn')") or die("ERROR QUERY");
?>
<script>
	swal({
    title: "Success!",
...
<div class="center_box_content">
	<form id="form1" name="form1" method="post" action="Signup_fac.php">
	 <div>
	 	<br>
	 	<center>
	 	<!-----------POSITION CHECK-------->
	 	<input type="radio" name="gender" value="Student" onclick="try1()" >Student
		<input type="radio" name="gender" value="Fac" onclick="try2()" checked>Faculty / Staff
		...
		<input type="text" class="form-control"  id="firstname" name="fname" value="<?php echo $fname; ?>" placeholder="<?php echo $fnameErr; ?>"/>
		...
		<input type="text" class="form-control" id="middlename" name="mname" value="<?php echo $mname; ?>" placeholder="<?php echo $mnameErr; ?>"/>
			...
			<label class="control-label">Last Name<span class="required">*</span></label>
					...
					<input type="date" placeholder="MM/DD/YY" name="bdate" value="<?php echo $bdate; ?>" class="datepicker" id="bdate" placeholder="<?php echo "<br> " . $bdateErr; ?>"/>
					...
					<label class="control-label">Department<span class="required">*</span></label>
							<div class="controls">
								<select name="dept">
									...
									<option value="AB English Language" <?php if($dept == "AB English Language"){ echo "selected";}?>>AB English Language</option>
									<option value="BS Elementary Education" <?php if($dept == "BS Elementary Education"){ echo "selected";}?>>BS Elementary Education</option>
									<option value="BS Architecture" <?php if($dept == "BS Architecture"){ echo "selected";}?>>BS Architecture</option>
									<option value="BS Civil Engineering" <?php if($dept == "BS Civil Engineering"){ echo "selected";}?>>BS Civil Engineering</option>
									<option value="BS Computer Engineering" <?php if($dept == "BS Computer Engineering"){ echo "selected";}?>>BS Computer Engineering</option>
									<option value="BS Secondary Education" <?php if($dept == "BS Secondary Education"){ echo "selected";}?>>BS Secondary Education</option>
									<option value="BS Electrical Engineering" <?php if($dept == "BS Electrical Engineering"){ echo "selected";}?>>BS Electrical Engineering</option>
									<option value="BS Information Technology" <?php if($dept == "BS Information Technology"){ echo "selected";}?>>BS Information Technology</option>
									<option value="BS Mechanical Engineering" <?php if($dept == "BS Mechanical Engineering"){ echo "selected";}?>>BS Mechanical Engineering</option>
								</select>
							</div>
				<br clear=both>
				<hr>
