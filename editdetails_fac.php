		...
		if(!empty($_POST["gender"]) && !empty($_POST["currentAdd"]) && !empty($_POST["bdate"])){
			if(!empty($_POST["civil"])){
				if(!empty($_POST["degree"]) && !empty($_POST["contactnum"])){
					...
					$query1 = mysql_query("UPDATE degree SET userDegree='$deg' WHERE userLRN='$lrn'") or die("ERROR QUERY");
						echo "<script> swal('Success!', 'User information successfully updated', 'success');</script>";
}}}}}
...
?>
<div class="container">
<form method="post" action="editdetails_fac.php">
...
<h4><?php echo $rowinfo[4]; ?></h4>
		<h5><a href="viewdetails_fac.php">View Profile</a></h5>
		</center>
		<hr>
